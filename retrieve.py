...
if __name__ == "__main__":
    ...
    download_dir = f"{download_dir}/raw"
    if not os.path.exists(download_dir):
        os.makedirs(download_dir)
    ...
    for offset in tqdm(range(0, args.total, 25), desc="download data"):
        ...
        for i, gif_data in enumerate(json_data['data']):
            ...
            with open(f'{download_dir}/{gif_id}_title.txt', 'w') as f:
                f.write(f"{title}\n")
            
            # get meta data
            meta_response = requests.get(gif_data["url"])
            soup = BeautifulSoup(meta_response.text, 'html.parser')
            meta_tag = soup.find("meta", attrs={"name": "keywords"})
            keywords = meta_tag.get("content")
            with open(f'{download_dir}/{gif_id}_keywords.txt', 'w') as f:
                f.write(f"{keywords}\n")
            f.close()
